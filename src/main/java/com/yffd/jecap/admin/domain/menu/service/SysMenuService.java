...
public class SysMenuService {
    ...
    public List<SysMenuTree> queryTree() {
        List<SysMenu> list = this.getDao().queryList(new SysMenu());
     ...
     * @return
     */
    public SysMenuTree queryTree(String menuId) {
        ...
        List<SysMenu> children = this.menuRepo.queryByPath(path);
        return SysMenuTree.buildTree(children, entity);
    }


    public void add(SysMenu menu) {
        if (null == menu || StringUtils.isBlank(menu.getMenuName())) throw SysException.cast("菜单名称不能为空");
    ...
    public void updateById(SysMenu menu, String pid) {
        ...
        if (StringUtils.isBlank(pid)) {
            ...
            //更改子节点path、pid
            List<SysMenu> children = this.queryChildrenByPath(curNode.getPath() + "," + curNode.getId());
    ...
    public void deleteById(String menuId) {
        ...
        // 其子节点ID
        Set<String> childrenIds = this.queryChildrenIds(menuId);
    ...
    public Set<String> queryChildrenIds(String orgId) {
        List<SysMenu> list = this.queryChildren(orgId);
    ...
    public List<SysMenu> queryChildren(String menuId) {
        ...
        return this.menuRepo.queryByPath(entity.getPath() + "," + entity.getId());
    }

    public List<SysMenu> queryChildrenByPath(String path) {
        if (StringUtils.isBlank(path)) return Collections.EMPTY_LIST;
        return this.menuRepo.queryByPath(path);
    }

    public SysMenu queryByPid(String pid) {
